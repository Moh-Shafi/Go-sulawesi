...
const navItems = [
  { id: 'home', to: '/business', label: { en: 'Home', id: 'Beranda' }, icon: (active: boolean) => (
  ...
  { id: 'listings', to: '/business/listings', label: { en: 'Listings', id: 'Daftar' }, icon: (active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? A : MUTED} strokeWidth="1.8">
      <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
      <circle cx="12" cy="9" r="2.5"/>
    </svg>
  )},
  { id: 'bookings', to: '/business/bookings', label: { en: 'Bookings', id: 'Pesan' }, icon: (active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? A : MUTED} strokeWidth="1.8">
      <rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/>
      <line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
    </svg>
  )},
  { id: 'settings', to: '/business/settings', label: { en: 'Settings', id: 'Pengaturan' }, icon: (active: boolean) => (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={active ? A : MUTED} strokeWidth="1.8">
...
export default function BusinessBottomNav() {
  ...
  const isActive = (to: string) => {
    if (to === '/business') return path === '/business' || path === '/business/'
